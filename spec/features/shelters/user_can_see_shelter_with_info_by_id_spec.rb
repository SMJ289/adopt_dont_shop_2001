require 'rails_helper'

RSpec.describe "shelter page", type: :feature do
  it "can show individual shelters with their details" do
    shelter_1 = Shelter.create(name: "Denver Animal Shelter", address: "1241 W Bayaud Ave", city: "Denver", state: "CO", zip: "80223")

    visit "/#{shelter_1.id}"

    expect(page).to have_content(shelter_1.title)
    expect(page).to have_content("Name: #{song_1.name}")
    expect(page).to have_content("Address: #{song_1.address}")
    expect(page).to have_content("City: #{song_1.city}")
    expect(page).to have_content("State: #{song_1.state}")
    expect(page).to have_content("Zip: #{song_1.zip}")
  end
end
